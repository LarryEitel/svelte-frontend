...
async function main() {
	...
	await prisma.activity.create({
		data: {
			...
			enrollmentStartDate: DateTime.now().minus({ days: 1 }).toISO(),
			enrollmentEndDate: DateTime.now().plus({ days: 1 }).toISO(),
			facultyId: facultyUnipampa.id,
	...
	await prisma.activity.create({
		data: {
			...
			enrollmentStartDate: DateTime.now().minus({ days: 3 }).toISO(),
			enrollmentEndDate: DateTime.now().minus({ days: 1 }).toISO(),
			facultyId: facultyUcs.id,
			targetAudience: TargetAudience.STUDENT,
			visibility: 'PUBLIC',
			modality: 'PRESENTIAL',
	...
	await prisma.activity.create({
		data: {
			...
			enrollmentStartDate: DateTime.now().minus({ days: 5 }).toISO(),
			enrollmentEndDate: DateTime.now().minus({ days: 3 }).toISO(),
			facultyId: facultyUnipampa.id,
			targetAudience: TargetAudience.STUDENT,
			visibility: 'PUBLIC',
			modality: 'REMOTE',
	...
	await prisma.activity.create({
		data: {
			...
			enrollmentStartDate: DateTime.now().minus({ days: 10 }).toISO(),
			enrollmentEndDate: DateTime.now().minus({ days: 7 }).toISO(),
			facultyId: facultyUcs.id,
	...
	for (let i = 0; i < 20; i++) {
		await prisma.activity.create({
			data: {
				...
				enrollmentStartDate: DateTime.now().minus({ days: 7 }).toISO(),
				enrollmentEndDate: DateTime.now().minus({ days: 5 }).toISO(),
				facultyId: facultyUnipampa.id,
				targetAudience: TargetAudience.EXTERNAL,
				visibility: 'PUBLIC',
				modality: 'REMOTE',
