...
export function GetReportsFromJSONTyped(json: any, ignoreDiscriminator: boolean): GetReports {
	...
	return {
		reports: !exists(json, 'reports')
			? undefined
			: (json['reports'] as Array<any>).map(GetReportsReportsInnerFromJSON)
	};
}
	...
	return {
		reports:
			value.reports === undefined
				? undefined
				: (value.reports as Array<any>).map(GetReportsReportsInnerToJSON)
	};
}
